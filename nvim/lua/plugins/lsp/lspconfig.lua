return {
	...
	config = function()
		...
		vim.lsp.handlers["textDocument/hover"] =
			vim.lsp.with(vim.lsp.handlers.hover, { border = "single", max_width = 100 })

		vim.lsp.handlers["textDocument/signatureHelp"] =
			vim.lsp.with(vim.lsp.handlers.signature_help, { title = "Signature", border = "single", max_width = 100 })
		...
		local lsp_servers = {
			...
			ruby_lsp = require("plugins.lsp.servers.ruby_lsp").setup(capabilities),
			rubocop = require("plugins.lsp.servers.rubocop").setup(capabilities),
		}
